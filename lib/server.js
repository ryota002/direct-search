...
async function searchLeads(searchParams) {
  ...
  return {
    ...
    message: leads.length
      ...
      : noLocation
      ? `${industry} の候補が少ないため、業種を具体的にするか拠点あり検索も試してください。`
      : `${location} 周辺の ${industry} は候補を取得できませんでした。検索地域や業種を変えて再検索してください。`,
    leads,
  };
}
...
function buildPlaceQueries(course, industry, location, profileKeywords = "", noLocation = false) {
  ...
  const industryTerms = industrySearchTerms(industry, course);
  const queries = [];

  industryTerms.forEach((term) => {
    const focusedCore = [term, locality, normalizedProfile].filter(Boolean).join(" ");
    const broadCore = [term, locality].filter(Boolean).join(" ");
    const coreVariants = normalizedProfile ? [focusedCore, broadCore] : [broadCore];

    coreVariants.forEach((queryCore) => {
      if (!queryCore) return;
      ...
      if (course === "sns") {
        queries.push(courseSpecificQuery, instagramQuery, baseQuery);
      } else {
        queries.push(baseQuery, courseSpecificQuery, instagramQuery);
      }
    });
  });
...
function industrySearchTerms(industry, course) {
  const normalized = String(industry || "").trim();
  const aliases = {
    "地域企業": ["地域密着 会社", "中小企業", "工務店", "不動産会社", "飲食店"],
    "地域密着企業": ["地域密着 会社", "中小企業", "工務店", "不動産会社", "飲食店"],
    "美容系": ["美容室", "ネイルサロン", "エステサロン", "美容サロン"],
    "士業": ["税理士事務所", "行政書士事務所", "社労士事務所", "司法書士事務所"],
    "士業・専門職系": ["税理士事務所", "行政書士事務所", "社労士事務所", "司法書士事務所"],
    "ハウスメーカー・工務店": ["工務店", "ハウスメーカー", "注文住宅"],
    "工務店・ハウスメーカー": ["工務店", "ハウスメーカー", "注文住宅"],
    "医療・クリニック系": ["クリニック", "歯科クリニック", "美容クリニック", "内科"],
  };

  const courseFallbacks = {
    design: ["クリニック", "サロン", "工務店", "不動産会社"],
    sns: ["美容室", "飲食店", "整体院", "パーソナルジム"],
    writing: ["オウンドメディア", "不動産会社", "人材会社", "IT企業"],
  };

  return [...new Set([...(aliases[normalized] || [normalized]), ...(courseFallbacks[course] || [])])];
}
