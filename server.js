...
const API_WINDOW_LIMIT = Number(process.env.API_WINDOW_LIMIT || 5);
const API_DAILY_LIMIT = Number(process.env.API_DAILY_LIMIT || 30);
...
async function handleRequest(req, res) {
  try {
    ...
    if (requestUrl.pathname === "/api/leads") {
      const limit = checkApiLimit(req, "leads");
    ...
    if (requestUrl.pathname === "/api/diagnose") {
      const limit = checkApiLimit(req, "diagnose");
      if (!limit.allowed) return sendRateLimit(res, limit);
...
if (require.main === module) {
  ...
  server.listen(PORT, HOST, () => {
    ...
    console.log(`API limit: ${API_WINDOW_LIMIT}/minute and ${API_DAILY_LIMIT}/day per IP`);
  });
}
...
function checkApiLimit(req, endpoint) {
  const now = Date.now();
  ...
  if (now - current.windowStartedAt >= API_WINDOW_MS) {
  ...
  const retryAfterSeconds = Math.max(1, Math.ceil((current.windowStartedAt + API_WINDOW_MS - now) / 1000));

  if (current.windowCount > API_WINDOW_LIMIT) {
    return {
      allowed: false,
      reason: "短時間に検索が集中しています。少し時間をおいてから再度お試しください。",
      retryAfterSeconds,
    };
  }

  if (current.dayCount > API_DAILY_LIMIT) {
    return {
      allowed: false,
      reason: "本日の利用上限に達しました。明日またお試しください。",
